...
function getHandlerForRequest({method, url, queryObj, body}) {
  ...
  if (configuredHandlers[method] && configuredHandlers[method][url] && configuredHandlers[method][url].length > 0) {
    ...
    const filtered = handlers
      .filter(conf => queryObjectsMatch(queryObj, conf.queryObj))
      .filter(conf => bodyObjectsMatch(body, conf.bodyObj))
...
function queryObjectsMatch(actualQO, configuredQO) {
  const queryObjectOrEmptyObject = actualQO || {}

  if (!configuredQO) {
    return true
  }

  return !Object.keys(configuredQO).find((key) => ('' + queryObjectOrEmptyObject[key]) !== ('' + configuredQO[key]))
}
